...
class API extends PDO
{
     ...
     * @param null $options PDO driver options
     */
    public function __construct(string $dsn, $user = '', $passwd = '', $options = null)
    {
     ...
     * @return PDOStatement
     */
    private function _prepare($sql, $bind = array()): PDOStatement
    {
     ...
     */
    public function run(string $sql, $bind = array()): int
    {
     ...
     * @return PDO
     */
    public function setFetchMode(string $fetchMode): PDO
    {
     ...
     */
    public function where(string $where, $andOr = 'AND'): string
    {
        if (is_array($where)) {
            $tmp = array();
     ...
     * @return integer Number of effected rows
     */
    public function bulkInsert(string $table, array $fieldNames, array $data, $replace = false): int
    {
     ...
     * @return integer Number of effected rows
     */
    public function update(string $table, array $data, $where = "", $bind = array()): int
    {
     ...
     * @return integer Number of effected rows
     */
    public function delete(string $table, string $where, $bind = array()): int
    {
     ...
     * @return integer Number of effected rows
     */
    public function save(string $table, array $data, $where = "", $bind = array()): int
    {
        $count = 0;
        if (!empty($where)) {
            $where = $this->where($where);
     ...
     */
    public function fetchOne(string $sql, $bind = array()): mixed
    {
        $stmt = $this->_prepare($sql, $bind);
     ...
     */
    public function fetchRow(string $sql, $bind = array()): array
    {
        $stmt = $this->_prepare($sql, $bind);
     ...
     */
    public function fetchAll(string $sql, $bind = array()): array
    {
        $stmt = $this->_prepare($sql, $bind);
        return $stmt->fetchAll($this->_fetchMode);
    }
     ...
     * @return array   Result rows
     */
    public function fetchAssoc(string $sql, $bind = array()): array
    {
        $stmt = $this->_prepare($sql, $bind);
     ...
     * @return array   Result rows
     */
    public function fetchAssocArr(string $sql, $bind = array()): array
    {
        $stmt = $this->_prepare($sql, $bind);
        $records = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $result = array();
